...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_turn(self, turn_state):
        ...
        self.put_shields_1(game_state)
        game_state.attempt_spawn(SCOUT,[[14,0]], 30)
        # # opening and closing - start
        # if self.needs_closing:
        #     game_state.attempt_spawn(TURRET, self.removed_turrets)
        #     self.needs_closing = False
        #     self.is_open = False
        #     # self.attack_left_side = not self.attack_left_side
        ...
        # self.repair_1(game_state)
        # self.upgrade_1(game_state)
        # self.put_shields_1(game_state)
        # self.repair_2(game_state)
        # self.upgrade_2(game_state)
        # self.put_shields_2

        game_state.submit_turn()

    def initial_setup_funnel(self, game_state):
        self.turret_init_points = [[9, 6], [10, 5], [11, 4], [12, 3], [13, 2], [14, 1]]
        game_state.attempt_spawn(SUPPORT, self.turret_init_points)
        game_state.attempt_upgrade(self.turret_init_points)
        # game_state.attempt_spawn(TURRET, [?[2, 12], [25, 12]])

    def attack_state(self, game_state):
        game_state.attempt_spawn(SCOUT, self.attack_coords,
                                 11 + (game_state.turn_number//7))
        game_state.attempt_remove([13,6])

    def prepare_attack(self, game_state):
        self.attack_left_side = self.choose_side(game_state)
        if self.attack_left_side: #attack on the left
            self.removed_turrets = self.removed_turrets_l
            self.attack_coords = self.attack_coords_l
        else:
    ...
    def put_shields_1(self, game_state):
        self.shields = [[14, 1], [13, 2], [12, 3], [11, 4], [10, 5], [9, 6], [8, 7], [7, 8], [6, 9], [5, 10], [4, 11], [3, 12], [2, 13]]
